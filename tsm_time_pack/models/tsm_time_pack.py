...
class TsmTimePack(models.Model):
    ...
    code = fields.Char(
        string="Time Pack Number", required=True, default="/", readonly=True
    )
    name = fields.Char(string="Time Pack Title", tracking=20, index=True, copy=False)
    description = fields.Html(
        string="Time Pack Description",
        sanitize=True,
        strip_style=False,
        translate=False,
        copy=False,
    ...
    _sql_constraints = [
        ("tsm_time_pack_unique_code", "UNIQUE (code)", _("The code must be unique!")),
    ]
    ...
    def _onchange_product_id(self):
        if self.product_id:
            ...
            product = self.product_id.with_context(
                lang=partner.lang, partner=partner.id, uom=self.product_uom_id.id
            )
    ...
    def _prepare_sale_line(self):
        self.ensure_one()
        name = _("{description_sale}\n Time Pack: [{code}] {name}").format(
            description_sale=self.description_sale, code=self.code, name=self.name or ""
        )
    ...
    def _prepare_sale(self):
        ...
        client_ref = _("Time Pack: [{code}] {name}").format(
            code=self.code, name=self.name or ""
        )
